...
class TerraformClient(Driver):
    ...
    def __init__(self, workspace: str, folder_path: str, terraform_bin_path: Optional[str] = None):
        ...
        self.init_return_code, self.init_stdout, self.init_stderr = self.tf.cmd(
            "-chdir=" + folder_path + " init -reconfigure -backend=true"
        )
        # self.init_return_code, self.init_stdout, self.init_stderr = self.tf.init(dir_or_plan=folder_path)
        ...
        if self.tf.show_workspace()[1].rstrip("\n") != workspace:
            ...
            # `while workspace not in self.tf.list_workspace():` this line should be commented out to
            # seperate the create
            # and set steps of the workspace part
            # while self.tf.show_workspace()[1].rstrip('\n') != workspace:
            self.workspace_return_code, self.workspace_stdout, self.workspace_stderr = self.tf.set_workspace(
                workspace=workspace
            )
    ...
    def apply(self, variables: Optional[dict] = None, parallelism: int = 10) -> Tuple[str, str, str]:
        ...
        return_code, stdout, stderr = self.tf.apply(
            no_color=IsFlagged, var=variables, skip_plan=True, parallelism=parallelism
        )
    ...
    def destroy(self, variables: Optional[dict] = None, parallelism: int = 10) -> Tuple[str, str, str]:
        ...
        return_code, stdout, stderr = self.tf.apply(
            no_color=IsFlagged, var=variables, skip_plan=True, parallelism=parallelism, destroy=True
        )
        return return_code, stdout, stderr
